/**
 * delete-2025-credit-debit-notes.ts
 *
 * Scans the credit_debit_notes table, finds every note whose date
 * or created_at falls in calendar year 2025, and batch-deletes them.
 *
 * Usage:  npx tsx src/delete-2025-credit-debit-notes.ts
 */
import { scanTable, batchDeleteItems, TABLES } from "./db/client.js";
import type { CreditDebitNote } from "./types/index.js";

async function main() {
  console.log("🔍 Scanning credit/debit notes for year 2025…\n");
  ...
  const toDelete = allNotes.filter((n) => {
    ...
    return dateYear === "2025" || createdYear === "2025";
  });
  ...
  console.log(`\n\n✅ Done — deleted ${deleted} credit/debit notes from year 2025.`);
}
