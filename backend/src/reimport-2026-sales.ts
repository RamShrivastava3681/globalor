 ...
 *
 * Usage:  npx tsx src/reimport-2026-sales.ts
 */
...
const EXPECTED_TOTAL = 14069201.18;
...
// ─── Parse sales2026-final.xlsx (simple format) ─────────────────────────────
...
function parseSimpleFormat(data: any[][]): ParsedRow[] {
...
async function main() {
  ...
  // ── Step 1: Delete all 2026 sales invoices ──
  console.log("🗑️  Deleting all 2026 sales invoices...\n");
  ...
  const inv2026 = allInvoices.filter((inv) => {
    const y = (inv.issue_date ?? "").slice(0, 4);
    return y === "2026";
  });
  console.log(`   Found ${inv2026.length} invoices for 2026`);

  if (inv2026.length > 0) {
    const keys = inv2026.map((i) => ({ id: i.id }));
    ...
    console.log(`\n   ✅ Deleted ${inv2026.length} 2026 sales invoices\n`);
  } else {
  ...
  for (const row of rows) {
    ...
    const invoice: Invoice = {
      ...
      short_payment: null,
      late_days: null,
      paid_note: "Reimported from sales2026-final.xlsx",
      status: "paid",
      noa_status: "not_sent",
  ...
  console.log(`\n═══════════════════════════════════════════════════════`);
  console.log(
    `📊 Total amount of invoices: $${roundedTotal.toLocaleString("en-US", { minimumFractionDigits: 2 })}`,
  );
  console.log(
    `📊 Expected total:           $${EXPECTED_TOTAL.toLocaleString("en-US", { minimumFractionDigits: 2 })}`,
  );
  ...
  if (Math.abs(roundedTotal - EXPECTED_TOTAL) > 0.01) {
    ...
    console.log(
      `   The import will proceed anyway — review the Excel data.`,
    );
  } else {
    console.log(`\n   ✅ TOTAL MATCHES!`);
  }
  ...
  console.log(
    `   Total amount:       $${roundedTotal.toLocaleString("en-US", { minimumFractionDigits: 2 })}`,
  );
  console.log(
    `   Expected total:     $${EXPECTED_TOTAL.toLocaleString("en-US", { minimumFractionDigits: 2 })}`,
  );
  if (Math.abs(roundedTotal - EXPECTED_TOTAL) <= 0.01) {
    ...
    console.log(
      `   Status:             ❌ MISMATCH (diff: $${(roundedTotal - EXPECTED_TOTAL).toLocaleString("en-US", { minimumFractionDigits: 2 })})`,
    );
  }
  console.log(`═══════════════════════════════════════════════════════`);
}
