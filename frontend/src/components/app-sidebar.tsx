...
export function useSidebarEntries(checkerCount = 0, queueCount = 0): Bucket[] {
  ...
  return useMemo(() => {
    const all: Entry[] = [
      ...
      { label: "My Workspace", icon: Briefcase, to: "/app/workspace", roles: ["checker", "treasury", "operations", "sales_rep"] },
      ...
      { label: "Customers", icon: Building2, to: "/app/customers", roles: ["sales_rep", "operations", "admin"] },
      ...
      { label: "My Reports", icon: Users, to: "/app/my-reports", roles: ["reporting_manager"] },
    ];
    ...
    const buckets: Bucket[] = [
      ...
      { id: "rep", label: "REPORTS & SYSTEM", entries: pick(["Reports", "My Reports"]) },
    ];
