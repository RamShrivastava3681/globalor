...
/* ═══════════ Cash Command Centre (shared: Finance tab + /app/cash alias) ═══════════ */
          ...
          <>
            ...
            {tab === "sales" && (
              <DataTable
                ...
                canManage={false}
                searchFn={(r, q) => `${r.invoice_number} ${r.customer?.name ?? ""} ${r.status}`.toLowerCase().includes(q)}
                ...
                columns={[
                  ...
                  { header: "Customer", render: (r) => r.customer?.name ?? "—", sortValue: (r) => r.customer?.name ?? "" },
  ...
  return (
    <div className="space-y-4">
      ...
      <DataTable
        ...
        empty="No invoices."
        canManage={false}
        searchFn={(r: any, q: string) => `${r.invoice_number} ${r.customer?.name ?? ""}`.toLowerCase().includes(q)}
        columns={[
          { header: "Invoice", render: (r: any) => <span className="font-mono text-xs">{r.invoice_number}</span>, sortValue: (r: any) => r.invoice_number },
          { header: "Customer", render: (r: any) => r.customer?.name ?? "—", sortValue: (r: any) => r.customer?.name ?? "" },
