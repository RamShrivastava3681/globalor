...
export function CustomersPage() {
  ...
  const [bulkPayCustomer, setBulkPayCustomer] = useState<any | null>(null);
    ...
    <div>
      ...
      <div className="p-6 md:p-10">
        ...
        <CounterpartyDashboard
          kind="customer"
          parties={(customersQ.data ?? []).map((d: any) => ({ id: d.id, name: d.name, industry: d.industry }))}
        ...
        <div className="mt-8">
          <div className="mb-3 flex items-center justify-between">
            <h3 className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.1em] text-muted-foreground">
              ...
              Customer directory ({(customersQ.data ?? []).length})
            </h3>
          </div>
        <Card>
                    ...
                    return (
                      <tr key={d.id} className="border-b border-border/60">
                        ...
                        <td className="px-5 py-3 text-right">
                          ...
                          {canPay && (
                            <button onClick={() => setBulkPayCustomer(d)} className="mr-2 rounded-md border border-primary/50 px-2 py-1 text-xs text-primary hover:bg-primary/10">
      ...
      {bulkPayCustomer && (
        <BulkPaymentModal
          ...
          invoices={(invoicesQ.data ?? []).filter((i: any) => i.customer_id === bulkPayCustomer.id)}
...
function CustomerFormModal({ editing, onClose, onDone }: { editing: any | null; onClose: () => void; onDone: () => void }) {
  ...
  const setAddr = (i: number, patch: Partial<AddressForm>) =>
    setAddresses((arr) => arr.map((a, idx) => (idx === i ? { ...a, ...patch } : a)));
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      for (const a of addresses) {
        if (!a.line1.trim() && !a.city.trim()) throw new Error("Every saved address needs at least a street or city");
      }
...
function BulkPaymentModal({ customer, invoices, onClose, onDone }: { customer: any; invoices: any[]; onClose: () => void; onDone: () => void }) {
  const qc = useQueryClient();
      ...
      <div className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-xl border border-border bg-card shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-border bg-card px-5 py-3">
          <div className="flex items-center gap-3">
            ...
            <h3 className="font-display text-lg">Bulk payment — {customer.name}</h3>
          </div>
          <button onClick={onClose} className="text-muted-foreground hover:text-foreground"><X className="h-4 w-4" /></button>
        </div>
        ...
        <div className="space-y-5 p-5">
          ...
          <div className="rounded-lg border border-border bg-background/40 p-4">
            ...
            {sortedInvoices.length === 0 ? (
              <div className="text-xs text-muted-foreground">No open invoices for this customer.</div>
            ) : (
              <div className="-mx-4 overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="text-xs uppercase tracking-widest text-muted-foreground">
