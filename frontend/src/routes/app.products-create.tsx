...
const GENDER_OPTIONS = ["Male", "Female", "Unisex", "Kids", "Boys", "Girls", "Infant"];

function buildPreviewSku(brand: string, gender: string, category: string, model: string): string {
  const b = (brand.trim().toUpperCase().slice(0, 2) || "AD").padEnd(2, "X");
  const gMap: Record<string, string> = { male: "M", female: "F", unisex: "U", kids: "K", boys: "M", girls: "F", infant: "I" };
  const gRaw = gMap[gender.trim().toLowerCase()] ?? gender.trim().charAt(0).toUpperCase();
  const g = gRaw || "M";
...
function CreateProductPage() {
  ...
  const [form, setForm] = useState({
    productName: "", itemNumber: "", brand: "AD", gender: "Male", category: "T-Shirt", modelNumber: "001",
    hsnCode: "", taxPercent: "18", unitOfMeasure: "Piece", unitCost: "600", unitPrice: "1000",
  });
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [skuConflict, setSkuConflict] = useState<{ suggestion?: string; msg?: string } | null>(null);
  const [confirmOpen, setConfirmOpen] = useState(false);

  const previewSku = useMemo(() => buildPreviewSku(form.brand, form.gender, form.category, form.modelNumber), [form.brand, form.gender, form.category, form.modelNumber]);
  ...
  useEffect(() => {
    if (!form.brand || !form.gender || !form.category || !form.modelNumber) return;
  ...
  }, [previewSku, form.brand, form.gender, form.category, form.modelNumber]);
  ...
  const create = useMutation({
    ...
    onError: (e: any) => {
      ...
      if (msg.includes("already in use") && msg.includes("Master SKU")) {
        const m = msg.match(/suggestion"?\s*:\s*"([^"]+)"/i);
        toast.error(msg);
        return;
      }
      // try to parse suggestion from error body
      toast.error(msg);
  ...
  const validate = (): boolean => {
    ...
    if (!/^[0-9]{4,8}$/.test(form.hsnCode.trim())) e.hsnCode = "HSN Code must be 4-8 digits";
  ...
  const applySuggestion = () => {
    ...
    // suggestion like AD-M-TS-002 -> extract model number
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Create the master product. The Master SKU is auto-generated as AD-GENDER-CATEGORY-MODEL_NUMBER and is unique system-wide."
      />
      ...
      <div className="mx-auto max-w-[1080px] p-4 md:p-8 space-y-6">
        ...
        <div className="rounded-xl border border-primary/20 bg-primary/[0.04] p-4 md:p-5">
          ...
          <div className="mt-1 font-mono text-[11px] text-muted-foreground">Format: AD-GENDER-CATEGORY-MODEL_NUMBER · e.g. AD-M-TS-001 · Auto-generated · Read-only</div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="rounded-xl border border-border bg-card p-5 md:p-6">
            ...
            <div className="grid gap-4 md:grid-cols-4">
              <F label="Brand *" error={errors.brand}><input placeholder="AD" maxLength={2} className={`inp font-mono ${errors.brand ? "border-destructive" : ""}`} value={form.brand} onChange={set("brand")} /></F>
              <F label="Gender *" error={errors.gender}><select className={`inp ${errors.gender ? "border-destructive" : ""}`} value={form.gender} onChange={set("gender")}>{GENDER_OPTIONS.map((g) => <option key={g} value={g}>{g}</option>)}</select></F>
